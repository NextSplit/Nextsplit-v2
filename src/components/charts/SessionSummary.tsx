    ...
    <div className="bg-white rounded-2xl border border-[var(--color-border)] p-5">
      ...
      <div className="grid grid-cols-4 gap-2">
        <div className="text-center">
          <div className="text-xl font-black text-[var(--ns-ember)]">{Math.round(units === 'miles' ? totalKm * 0.621371 : totalKm)}</div>
          <div className="text-[10px] text-[var(--color-text-tertiary)] mt-0.5">{units === 'miles' ? 'mi' : 'km'}</div>
        </div>
        <div className="text-center">
          <div className="text-xl font-black text-gray-900">{totalSessions}</div>
          <div className="text-[10px] text-[var(--color-text-tertiary)] mt-0.5">sessions</div>
        </div>
        <div className="text-center">
          <div className="text-xl font-black text-gray-900">
            {totalHours > 0 ? totalHours.toFixed(1) : '—'}
          </div>
          <div className="text-[10px] text-[var(--color-text-tertiary)] mt-0.5">hours</div>
        </div>
        <div className="text-center">
          <div className="text-xl font-black text-gray-900">
            {avgEffort > 0 ? avgEffort.toFixed(1) : '—'}
          </div>
          <div className="text-[10px] text-[var(--color-text-tertiary)] mt-0.5">avg RPE</div>
        </div>
      </div>
      {plannedTotal > 0 && (
        <div className="mt-4">
          ...
          <div className="h-1.5 bg-[var(--color-surface-2)] rounded-full overflow-hidden">
            <div
              className="h-full bg-[var(--ns-ember)] rounded-full"
