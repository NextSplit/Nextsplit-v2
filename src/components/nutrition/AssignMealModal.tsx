    ...
    <div className="fixed inset-0 z-50 flex items-end" onClick={onClose}>
      <div className="w-full max-w-lg mx-auto bg-white rounded-t-3xl shadow-2xl max-h-[85vh] flex flex-col"
        ...
        <div className="overflow-y-auto flex-1 p-6">
          ...
          <p className="text-xs text-[var(--color-text-tertiary)] mb-4">Choose a recipe from your repository</p>

          {recipes.length === 0 ? (
            <div className="text-center py-8">
              <p className="text-sm text-[var(--color-text-tertiary)]">No recipes yet.</p>
              <p className="text-xs text-[var(--color-text-tertiary)] mt-1">Add one in the Recipes tab first.</p>
            </div>
          ) : (
              ...
              {recipes.map(r => {
                ...
                return (
                  <button key={r.id}
                    ...
                    className={`w-full text-left rounded-2xl border px-4 py-3 transition-all ${selected?.id === r.id ? 'border-[var(--ns-ember)] bg-[var(--ns-ember-light)]' : 'border-[var(--color-border)] bg-white'}`}
                  >
          ...
          {selected && (
            <div className="mt-4 pt-4 border-t border-[var(--color-border)]">
              <label className="text-xs font-semibold text-[var(--color-text-secondary)] block mb-2">
                Portions <span className="text-[var(--color-text-tertiary)] font-normal">(how many servings for this meal)</span>
              </label>
