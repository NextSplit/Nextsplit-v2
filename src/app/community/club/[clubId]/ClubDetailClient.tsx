    ...
    <div className="min-h-screen bg-slate-50 pb-24">
      ...
      <div className="bg-white border-b border-slate-100 px-4 pt-12 pb-4 sticky top-0 z-40">
        <div className="max-w-lg mx-auto space-y-3">
          <div className="flex items-center gap-3">
            ...
            {(isOwner || membership) && (
              <button onClick={() => setShowCode(!showCode)}
                className="text-xs bg-teal-50 border border-teal-200 text-teal-700 px-3 py-1.5 rounded-xl font-bold">
                Invite
              </button>
            )}
          </div>
          ...
          {showCode && (
            <div className="bg-teal-50 border border-teal-200 rounded-2xl p-3 space-y-2">
              <p className="text-xs text-teal-700 font-semibold">Share this code to invite members:</p>
              <div className="flex items-center gap-2">
                <code className="flex-1 text-center text-2xl font-black text-teal-800 tracking-widest">{club.join_code}</code>
                <button onClick={copyCode}
                  className={`px-3 py-1.5 rounded-xl text-xs font-bold ${copied ? 'bg-emerald-500 text-white' : 'bg-teal-500 text-white'}`}>
                  {copied ? '✓' : 'Copy'}
                </button>
              </div>
            </div>
                ...
                <div key={m.user_id}
                  ...
                  <div className="flex-1 min-w-0">
                    <p className={`text-sm font-bold truncate ${isMe ? 'text-teal-800' : 'text-slate-900'}`}>
                      {name} {isMe && '(you)'}
                      {m.role === 'owner' && <span className="ml-1 text-[10px] text-teal-500">Owner</span>}
                    </p>
                    <p className="text-[10px] text-slate-400">{LEAGUE_CONFIG[league]?.emoji} {m.season_xp} XP this season</p>
                  </div>
                  <div className="text-right shrink-0">
                    <p className={`text-sm font-black ${isMe ? 'text-teal-700' : 'text-slate-700'}`}>{Math.round(m.weekly_km)}km</p>
