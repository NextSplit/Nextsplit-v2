      ...
      <div className="max-w-lg mx-auto px-4 py-4 space-y-3">
        ...
        {!planLoading && !plan && activeTab === 'today' && (
          <div className="bg-white rounded-2xl border border-gray-100 p-8 text-center">
            ...
            <a href="/onboarding"
              className="inline-block bg-[var(--ns-ember)] text-white px-6 py-2.5 rounded-xl text-sm font-semibold">
              Choose a plan →
            </a>
          </div>
        )}
        ...
        {activeTab === 'week' && (
          <>
            ...
            {(() => {
              ...
              return (
                <div className="bg-[var(--ns-cyan-light)] rounded-2xl border border-teal-100 p-4">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-sm">🏅</span>
                    <span className="text-[11px] font-bold text-teal-800 uppercase tracking-wide">Cross-training today</span>
                    {extraKcal > 0 && <span className="ml-auto text-[11px] font-bold text-[var(--ns-ember)]">+{extraKcal} kcal</span>}
                  </div>
                  <div className="space-y-1">
                    {todayActivities.map((a: { id: string; activity_type: string; duration_secs?: number | null; notes?: string | null }) => (
                      <div key={a.id} className="flex items-center gap-2 text-xs text-[var(--ns-ember)]">
                        ...
                        {a.duration_secs && <span className="text-[var(--ns-cyan-mid)]">{Math.round(a.duration_secs / 60)}min</span>}
                        {a.notes && <span className="text-[var(--ns-cyan-mid)] truncate max-w-[120px]">{a.notes}</span>}
                      </div>
                    ))}
                  </div>
                </div>
        ...
        {activeTab === 'recipes' && (
          <>
            <button onClick={() => { setEditingRecipe(null); setShowRecipeForm(true) }}
              className="w-full py-3 rounded-2xl bg-[var(--ns-ember)] text-white text-sm font-semibold active:scale-95 transition-transform">
              + New recipe
            </button>
            {recipes.length === 0 ? (
              <div className="bg-white rounded-2xl border border-gray-100 p-8 text-center">
                ...
                <button
                  ...
                  className="inline-block bg-[var(--ns-ember)] text-white px-5 py-2 rounded-xl text-sm font-semibold">
                  Add first recipe →
                </button>
              </div>
            ) : (
