...
function StarRating({ rating, count }: { rating: number | null; count: number }) {
  if (!rating || count === 0) return <span className="text-xs text-gray-400">No ratings yet</span>
  return (
    ...
    <div className="min-h-screen pb-24" style={{ background: '#f8f8f6' }}>
      <div className="bg-white border-b border-gray-100 px-4 pt-12 pb-4 sticky top-0 z-40">
        <div className="max-w-lg mx-auto flex items-center justify-between">
          <div>
            <Link href="/coach/squad" className="text-xs text-gray-400 hover:underline">← Squad</Link>
      ...
      <div className="max-w-lg mx-auto px-4 pt-4 space-y-4">
                ...
                <div key={plan.id} className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
                  <div className="px-4 pt-4 pb-3">
                    <div className="flex items-start justify-between gap-2 mb-2">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-gray-400">{plan.distance} · {plan.level}</p>
                      </div>
                      <div className="flex items-center gap-1.5 flex-shrink-0">
                        ...
                        {price && (
                          <span className="text-[10px] font-bold text-gray-600">£{price}</span>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="grid grid-cols-3 divide-x divide-gray-100 border-t border-gray-100">
                    <div className="px-3 py-2.5 text-center">
                      ...
                      <p className="text-[10px] text-gray-400">starts</p>
                    </div>
                    <div className="px-3 py-2.5 text-center">
                  ...
                  <div className="flex border-t border-gray-100">
                    <Link href={`/marketplace/${plan.id}`}
                      className="flex-1 py-3 text-center text-xs font-bold text-gray-500 hover:bg-gray-50">
                      View listing
                    </Link>
        ...
        {!isPro && (
          <div className="bg-white rounded-2xl border border-gray-100 p-4">
            <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Marketplace revenue</p>
