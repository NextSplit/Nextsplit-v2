...
<!-- 9.9: Audit + 11-agent /council pass complete on `claude/review-project-status-lGBPu`. Two deliverables on main via PR #12: docs/audit/audit-report-v1.md (1368 lines, Phases 0-9 + Phase 8 council synthesis, 39 findings + 14 council additions S1-S14) and docs/audit/roadmap-integration-v1.md (every finding → 8-track plan). Aggregate verdict: HOLD-WITH-IMMEDIATE-HOTFIX. F2.1 RLS pre-flight ran live: only `plan_templates` returned rls_enabled = false; everything else clean — that single-table escalation is patched in supabase/migrations/phase-track1-hotfix-v1.sql (pending MCP-apply). Track 1 hotfix code-side shipped this session: F0.1 (deploy.yml deleted), F0.3 (admin gate via ADMIN_EMAILS env var on src/app/admin/retention/page.tsx), S12 (manifest.json background_color #0a0e1a), S3 (can_nudge SECURITY DEFINER guard applied live to Supabase). Track 1 SQL remaining: phase-track1-hotfix-v1.sql via Supabase MCP (F2.1 plan_templates RLS + F2.2 nps_responses leak). Supabase MCP + Vercel API token both wired in .claude/settings.local.json (gitignored). Supabase project ref: wlrmeiczqgmharvfmalq. Vercel token: full-account scope. ADMIN_EMAILS confirmed deployed by founder. **BOTH TOKENS PASTED IN CHAT — ROTATE BOTH IMMEDIATELY in next session before any work: supabase.com/dashboard/account/tokens + vercel.com/account/tokens.** Founder admin still: ICO registration (£40, ico.org.uk) — legal council reclassified F2.10 INFO → RED. NEXT GATING EVENT: F1 friend test (P1.8). NEXT WORK: Track 1 SQL apply via MCP, then Track 1.5 (S6 aiRateLimit on 5 unguarded AI routes, S5 gen-types.sh path fix, F4.1 database.ts regen, S10 unified onboarding events). -->
...
## ⚠️ Supabase MCP + Vercel API access wired — rotate BOTH tokens

`.claude/settings.local.json` (gitignored) holds:

- **Supabase MCP server** — project ref `wlrmeiczqgmharvfmalq`, PAT in `mcpServers.supabase.env.SUPABASE_ACCESS_TOKEN`. Tools available next session: `mcp__supabase__list_tables`, `mcp__supabase__execute_sql`, `mcp__supabase__apply_migration`, `mcp__supabase__list_migrations`, `mcp__supabase__generate_typescript_types`.
- **Vercel API token** — full-account-scoped, in top-level `env.VERCEL_TOKEN`. No MCP server (Vercel doesn't ship a stable bearer-auth MCP yet). Use directly via `curl` in Bash — token is in `$VERCEL_TOKEN`. Examples:

  ```bash
  # whoami / verify token
  curl -s -H "Authorization: Bearer $VERCEL_TOKEN" https://api.vercel.com/v2/user
  # list projects (find NextSplit project ID)
  curl -s -H "Authorization: Bearer $VERCEL_TOKEN" https://api.vercel.com/v9/projects | jq '.projects[] | {id, name}'
  # list env vars on a project
  curl -s -H "Authorization: Bearer $VERCEL_TOKEN" "https://api.vercel.com/v10/projects/<projectId>/env" | jq '.envs[] | {key, target}'
  # trigger production redeploy
  curl -s -X POST -H "Authorization: Bearer $VERCEL_TOKEN" -H "Content-Type: application/json" \
    "https://api.vercel.com/v13/deployments" \
    -d '{"name":"nextsplit-v2","project":"<projectId>","gitSource":{"type":"github","repoId":<repoId>,"ref":"main"}}'
  ```

**Both tokens were pasted in chat this session — rotate FIRST THING in the next session:**

1. **Supabase:** <https://supabase.com/dashboard/account/tokens> → revoke `sbp_dbc28732…` → create new project-scoped → swap into `.claude/settings.local.json` → `mcpServers.supabase.env.SUPABASE_ACCESS_TOKEN`.
2. **Vercel:** <https://vercel.com/account/tokens> → revoke `vcp_7ZulRjsB…` → create new (team-scoped if you have a team; full-account otherwise) → swap into `.claude/settings.local.json` → `env.VERCEL_TOKEN`.
3. Restart Claude Code so both reload. Verify with the `whoami` curl above + a Supabase MCP `list_tables` call.

**Vercel token has wider blast radius than Supabase** — full account = read every env var (incl. `SUPABASE_SERVICE_ROLE_KEY`, `STRIPE_SECRET_KEY`, `ANTHROPIC_API_KEY`), modify domains, deploy/rollback. Treat leakage as tier-1 incident.
