...
**Version:** 9.37 | **13 May 2026** | **Canonical — replaces all previous HANDOFF files**
<!-- 9.37: PR J9d — Mixamo asset conversion + rigged character swap. **Pipeline executed**: founder uploaded 9 FBX files to branch `raw-3d-fbx` (3 body types × 3 animations from Mixamo). To get under GitHub's 25 MB upload limit on mobile, the 6 non-idle FBXes were re-downloaded "Without Skin" (animation data only, no mesh) — 348-482 KB each; idle FBXes carry the mesh at 594-900 KB. Sandbox conversion: (1) Meta's `fbx2gltf` 0.9.7 binary converted each FBX → GLB; (2) custom merge script using `@gltf-transform/core` opened idle.glb as canonical doc, iterated over running.glb + celebrating.glb animations, copied each channel's sampler input/output accessors, and rebound channels to base-doc nodes via bone-name lookup (mixamorig:Hips, etc.). All 53 (running) + 52 (celebrating) channels rebound cleanly, ZERO missing targets — Mixamo's skeleton is consistent across stock characters. (3) Output GLBs: character-lean 376 KB, character-stocky 373 KB, character-tall 296 KB (all within 400 KB target). All have three animation clips named exactly 'idle', 'running', 'celebrating' (matching what Character3D's useAnimations[state] expects). **Client swap in `src/components/rpg/Character3D.tsx`**: replaced the procedural-humanoid default export with a Suspense-wrapped composition — `Character3DRigged` uses `useGLTF('/3d/character-${body}.glb')` + `useAnimations` with 0.2s crossfade between states, plus J9c cosmetics still layered on top (aura halo via Mesh + class-tint emissive on traversed skinned meshes when classHex set). `Character3DProcedural` kept in the same file as the Suspense fallback for asset-load failures (404/network) and any future body type without a baked GLB. `useGLTF.preload()` called at module top for all three paths so cold-load network is hidden by the dynamic-import waterfall. Scene cloned per-mount (`scene.clone(true)`) so HeroCard + LevelUpScreen rendering simultaneously don't share skeleton state. **Trade-off**: m1/f1 both have body='lean' and now both load character-lean.glb — they look identical visually. Within-body character variety (skin/hair per RPG_CHARS) is no longer reflected in the rigged mesh, only in the aura/rim hints. Founder can add per-character Mixamo variants later (character-lean-m1.glb, character-lean-f1.glb etc.) by extending GLB_PATH; non-breaking change. Type-check clean. **No new founder action**. **Verification**: hard-refresh `/home` — hero card should show a properly proportioned Mixamo character with idle breathing animation. Walk to /train (or trigger celebrating state) — character should crossfade animations smoothly. /dev/3d → cycle body types → lean/stocky/tall load distinct Mixamo characters. **Cleanup**: raw FBXes in branch `raw-3d-fbx` (PR #93) can be closed without merging — they served only as transfer; they're not used after sandbox conversion. **Next**: J9 chain complete. Remaining: J11b (Garmin), J13 (ElevenLabs), J6 (edge functions). -->
<!-- 9.36: PR J9b + J9c — wired Character3D into HeroCard + LevelUpScreen + extended Character3D with cosmetics (aura, class shaders, kit trim). **Scope decision**: J9b plan listed "HeroCard / RaceReplay / LevelUp", but RaceResultReplay uses emoji-on-lane intentionally — it renders 8+ runners in a CSS horizontal-progress-bar viz, and putting 8+ WebGL canvases on one page would tank mobile FPS for no visual win. Skipped. CharacterProfileModal was a candidate but already uses RunnerClassAvatar (a separate illustrated-avatar component), and the renderCharSVG line was dead code. Skipped. **Integration**: (1) new `src/components/rpg/Character3DDynamic.tsx` — shared `next/dynamic({ ssr: false })` wrapper with SVG fallback layered underneath (visible while three.js loads, covered once R3F mounts so layout doesn't jump). Includes CSS-variable resolver — `RPG_CHARS[].accent` uses `var(--ns-cyan)` for m1/f1; three.js can't parse CSS vars, so the wrapper resolves the 5 in-use design-system vars (`--ns-cyan` `#00d4ff`, `--ns-ember` `#ff5e3a`, `--ns-track` `#22c55e`, `--ns-magenta` `#ec4899`, `--ns-amber` `#f59e0b`) to their hex equivalents. Exposes `forceSvg` prop for performance-critical small previews. (2) HeroCard — replaced the `dangerouslySetInnerHTML={{ __html: renderCharSVG(...) }}` block + outer `animate-bounce` (the canvas plays its own V-jump in celebrating state) with `<Character3DDynamic size=84>`. Wires `runnerColour` → `classHex` and `stats.level.tier` → `tier`. (3) LevelUpScreen — replaced the 120x150 SVG with `<Character3DDynamic state="celebrating" size=150 tier={rpgLevel.tier}>`. **J9c cosmetics added to Character3D.tsx**: (a) **aura halo** — torus around character's feet, rendered only when `tier > 0`. Pulses at `0.5 + tier * 0.4` Hz with `0.10 + tier * 0.05` scale swing + rotates at 0.4 rad/s. Tinted by `classHex ?? kitHex`. Opacity ramps from 0.18 (tier 1) to 0.48 (tier 3). (b) **class shader rim** — Canvas's rim directional light adopts `classHex` when set; kit material gets a 10%-intensity emissive of the same hue. Marathon Runner reads visibly distinct from Speed Merchant even at HeroCard's 84px size. (c) **kit trim** — silver torus around shoulders at tier 2, gold at tier 3; metalness ramp on kit material (0 → 0.15 → 0.5 → 0.9 across tiers) so high-level kit reads glossier. **Dev preview** at `/dev/3d` now has tier buttons (T0/T1/T2 silver/T3 gold) + class-colour picker with "None" toggle. Type-check clean. **No founder action**. **Verification**: visit `/home` — hero card should show the 3D character (SVG flash on slow networks, then WebGL takeover). Walk through tiers in /dev/3d → aura should appear at T1+, ring should be silver at T2, gold at T3, faster + bigger pulse at higher tiers. Set a class colour → rim light + aura tint match. **Next**: J9 chain complete. Remaining: J11b (Garmin — founder needs to register at developer.garmin.com first), J13 (ElevenLabs — needs API key), J6 (edge functions — conditional). -->
<!-- 9.35: PR J9a — 3D character foundation shipped (procedural placeholder, no Mixamo asset yet). Goal: get the rendering pipeline, lighting, animation state-machine and code-site shape locked down so J9b just has to swap the placeholder mesh body for a `useGLTF`+`useAnimations` pair when the founder drops a rigged glTF. **New deps**: `three@0.184.0`, `@react-three/fiber@9.6.1`, `@react-three/drei@10.7.7`, `@types/three`. Bundle hit ~600 KB gz total — kept entirely off the initial bundle via `next/dynamic({ ssr: false })` at every call-site (currently just /dev/3d). **New components**: (1) `src/components/rpg/Character3D.tsx` — procedural humanoid built from capsule torso, sphere head, capsule limbs; varied by body type from `RPG_CHARS` (lean / stocky / tall — 6 height/width/shoulder shapes), parameterised by skin/hair/kit colours; animations computed inline in `useFrame` as sin-wave joint rotations keyed on `state` ('idle' = 1Hz breathing bob, 'running' = 2Hz arm/leg counter-swing 0.9/0.7 rad with footstrike bounce, 'celebrating' = V-jump with arms raised). TODO comment marks the exact lines J9b will swap with `useGLTF('/3d/character-${body}.glb')` + `useAnimations(actions[state]?.reset().fadeIn(0.2).play())`. (2) `src/components/rpg/Character3DCanvas.tsx` — wraps Character3D in an R3F `<Canvas>` with three-point lighting (white key + cyan fill + kit-colour rim), alpha+antialiased GL, dpr=[1,2] for retina. Optional `interactive` prop enables OrbitControls for the dev preview. (3) `src/app/dev/3d/page.tsx` + `Character3DPreviewClient.tsx` — admin-only (`ADMIN_EMAILS` gate, redirects non-admins to /home) preview at /dev/3d. UI: 320px square canvas with cyan radial glow, character picker (6 chars), animation picker (3 states), HTML5 colour picker for kit hex. **No founder action required** to ship — placeholder is visually coherent enough for the foundation. **Asset prep when ready**: founder generates Mixamo download for each body type (lean/stocky/tall), exports as glTF Binary (.glb), drops into `/public/3d/character-{body}.glb`; J9b is the one-line swap. Type-check clean. **No call-sites integrated yet** — HeroCard / RaceReplay / LevelUp swap is J9b. **Next**: J9b (wire to HeroCard) → J9c (cosmetics + class shaders). After J9 chain, options are J11b (Garmin — needs founder API key) or J13 (ElevenLabs — needs founder API key) or J6 (edge functions — conditional). -->
<!-- 9.34: PR J11c — file-import (.fit / .tcx) fallback for users who don't connect Strava or Garmin. New `src/lib/fileImport.ts` with two parsers: (1) `parseFit(buffer)` — uses `fit-decoder` (288kB unpacked, zero-dep) to extract the binary FIT session record's `total_distance` + `total_elapsed_time` + `avg_heart_rate` + `start_time`; (2) `parseTcx(xml)` — uses `fast-xml-parser` to walk Garmin Training Center XML `Activities > Activity > Lap[]`, sums distance + time across laps, averages HR weighted by lap, builds per-lap splits with pace in s/km. Format detection via filename extension + magic-byte fallback (`.FIT` at offset 8 for FIT, `<` or BOM for XML). New endpoint `POST /api/import/file` accepts multipart/form-data with `file` field, max 5 MB. Idempotent: rejects upload if a `training_logs` row exists for this user within ±5 min of the activity's `start_time` (catches re-uploads of the same activity). Returns 409 Already Imported with the existing log ID, or 200 with `{ log, parsed }` on success. New `src/components/FileImportSection.tsx` mounts under StravaSection on `/profile` — single "📁 Choose file" button via hidden `<input type=file accept=".fit,.tcx">`; on success renders a green confirmation card with km + duration + avg HR + start-time. Deps added: `fit-decoder@^1.0.5`, `fast-xml-parser@^5.8.0`. Type-check clean. **No new founder action**. **Verification**: export a workout from Garmin Connect, Apple Health, or Wahoo as either format → /profile → "Import a run" → Choose file → row appears in `training_logs` within ~2s. **Next**: J9 (3D character — needs Mixamo asset prep), J11b (Garmin Connect — needs founder Garmin Developer registration), J13 (ElevenLabs — needs API key), or J6 (edge functions — conditional). -->
<!-- 9.33: PR J16 — Supabase Realtime fully activated for squad surfaces. **Production bug discovered**: the `supabase_realtime` publication was EMPTY, so the existing SquadFeed.tsx INSERT listener (PR P1.1, May 2026) was silently no-op'ing — squad cards only appeared after a manual page reload, even though the client-side subscription was technically working. Migration `phase-squad-realtime-v1.sql` (applied live) adds 3 tables to the publication: `squad_feed` (new milestone cards — fixes the dormant listener), `squad_feed_reactions` (new in J16 — reactions from other squad-mates now appear live), `notifications` (new in J16 — cron-fired pushes appear without reload). `REPLICA IDENTITY FULL` set on `squad_feed_reactions` so DELETE events carry the pre-delete row data (default replica identity only broadcasts the PK column — without FULL, the client couldn't resolve which `feed_item_id` an un-react event belonged to). **Client changes**: SquadFeed.tsx extends the existing channel with 2 new postgres_changes listeners — `squad_feed_reactions` INSERT (skips own user_id since toggleReaction already applies optimistically; updates the matching feed row's reactions array; de-dupes by user_id) and `squad_feed_reactions` DELETE (filters by feed_item_id + user_id from the FULL replica payload; removes the reaction). `useNotifications.ts` gains a 2nd useEffect that subscribes to `notifications` INSERT filtered by `user_id=eq.<current>`; new rows prepend to the unread list capped at 5. RLS continues to gate delivery — non-squad-members can't receive other squads' rows; users can't receive other users' notifications. Type-check clean. **No new founder action**. **Verification**: open `/squad` on two devices signed in as different squad-mates. On device A, react to a card → device B sees the reaction appear within ~500ms. On device A, log a session that fires a feed card → device B sees the card prepend without reload. **Next**: J11b (Garmin) or J9 (3D character) — both need founder external setup; J11c (file-import) and J13 (ElevenLabs) are also queued. -->
<!-- 9.32: PR J15 — Resend + React Email infrastructure. Resend was already wired (lifecycle-emails cron uses it via raw HTML strings) but transactional templates were hand-rolled HTML. J15 adds a proper React Email pipeline: (1) New `src/lib/email.ts` with `sendEmail({to, subject, react, marketing})` that renders a React Email component to HTML via `@react-email/render` and posts via `resend.emails.send`. Includes RFC 8058 `List-Unsubscribe` + optional `List-Unsubscribe-Post: List-Unsubscribe=One-Click` header for marketing emails. Graceful no-op when `RESEND_API_KEY` unset (returns `{ok: false, error: 'RESEND_API_KEY not set'}`). (2) New `src/emails/_chrome.tsx` shared chrome component (navy header bar with ember stripe, light body for inbox legibility, footer with unsubscribe link). (3) Three templates: `StreakSave` (day-5 no-log gentle nudge, "consistency beats perfection"), `CoachInvite` (athlete → coach OAuth-style accept link with 7-day expiry copy), `FounderOnboarding` (personal welcome at sign-up — plain-text feel + thesis-restating "someone waiting for you" + reply-to-me CTA). Deps: `@react-email/components` + `@react-email/render`. Type-check clean. **No new founder action for J15** — `RESEND_API_KEY` is already set per existing lifecycle-emails cron. Future PRs can simply `sendEmail({to, subject, react: <StreakSave .../>}` to fire any branded transactional email. **Next**: PR J11b — Garmin Connect OAuth + activity import. -->
<!-- 9.31: PR J12 — OpenWeatherMap pre-run forecast in `PreRunBrief.tsx`. New `src/lib/weather.ts` wraps the OpenWeatherMap one-call v3 API (`api.openweathermap.org/data/3.0/onecall?lat,lon`, free tier 1000 calls/day) with 60-min Vercel ISR cache. `getWeather(lat, lon, atTs)` returns a `WeatherSnapshot` (temp, feels_like, humidity, wind kph, condition, icon) or `null` when `OPENWEATHER_API_KEY` is missing (graceful no-op). `getWeatherAdvice(snapshot)` computes running-specific advice: pace adjustment (3-4 s/km per °C above 16°C, or 30s/km slower start below 0°C), wind warning (≥15 kph "into wind first"; ≥25 kph "out-and-back"), layering recommendation tiered by feels-like temp + conditions overlay (rain → "+rain jacket", snow → ice-warning). New cached endpoint `/api/weather?lat=&lon=&at=?` (auth-required to prevent free-tier scraping). `PreRunBrief.tsx` gains a `<WeatherStrip />` that requests `navigator.geolocation` on mount (30min maxAge cache) and renders the strip between Coach Cue and Warmup blocks — falls silent when geolocation is denied or weather API not configured. **CSP + Permissions-Policy updated**: `img-src` adds `https://openweathermap.org` for the icon images; `Permissions-Policy: geolocation=(self)` (was `geolocation=()` which blocked the strip entirely). Type-check clean. **Founder action (J12, ~5 min)**: (a) sign up at openweathermap.org/api (free); (b) wait ~10 min for the API key to activate; (c) set `OPENWEATHER_API_KEY` on Vercel Production + Preview. Until set, the strip stays hidden — zero visual regression. **Next**: PR J15 — Resend + React Email templates. -->
<!-- 9.30: PR J11a — Strava webhook receiver at `/api/strava/webhook`. Was missing entirely (only OAuth callback + on-demand sync existed). Now: GET handles the Strava subscription verification handshake (echoes `hub.challenge` when `hub.verify_token` matches `STRAVA_WEBHOOK_VERIFY_TOKEN`); POST handles activity events. On `aspect_type='create' object_type='activity'`, the handler does a fire-and-forget background import — looks up the user by Strava athlete_id, refreshes the OAuth token if needed, fetches the activity detail, idempotently inserts a `training_logs` row (deduped via `strava_id` like the existing sync flow). Strava expects 200 in <2s so the response returns immediately while the import runs. update + delete aspect types currently `{ignored: true}` — wire in a follow-on if needed. New env entry `stravaWebhookVerifyToken` in `config.ts`. Type-check clean. **Founder action (J11a, ~10 min)**: (a) set `STRAVA_CLIENT_ID`, `STRAVA_CLIENT_SECRET`, `STRAVA_WEBHOOK_VERIFY_TOKEN` (any random string) on Vercel Production + Preview; (b) after deploy lands, register the webhook subscription with Strava: `curl -X POST https://www.strava.com/api/v3/push_subscriptions -F client_id=<id> -F client_secret=<secret> -F callback_url=https://nextsplit.app/api/strava/webhook -F verify_token=<TOKEN>` — Strava will GET our endpoint to verify, then start POSTing activity events. (c) Test by completing a Strava activity — should appear in `training_logs` within ~60s. **Next**: PR J12 — OpenWeatherMap pre-run forecast. -->
<!-- 9.29: PR J8 — pre-merge hygiene process documented in CLAUDE.md. Zero LOC of app code; pure process change. Added a "Pre-merge hygiene" section to CLAUDE.md covering three skills: `/security-review` REQUIRED on PRs touching payments / auth / DB schema / AI endpoints / user-input handling; `/review` RECOMMENDED on PRs >300 LOC or cross-cutting refactors; `/council` for proposals/design questions before code. Audit precedent cited: PR G3's P0 bug (missing notifications table referenced by the cron, silently failing for days) would have been caught by a `/security-review` pass on the relevant PR. Skill outputs stay advisory — review findings, fix the actionable ones, document deliberate "won't fix" in the PR body. **Phase A + B both COMPLETE** as of this PR: J1, J2, J7, J10, J17, J14, J4, J3, J5, J8 (10 PRs shipped). **Next: Phase C (data ingestion + comms)** starting with PR J11a — Strava env-var unlock + webhook hardening. -->
<!-- 9.28: PR J5 — Vercel toolbar feedback admin surface at `/admin/feedback`. Toolbar comments aren't reachable from runtime (Vercel's API is MCP-internal), so this uses the same snapshot pattern as J1 advisors: `src/lib/feedbackSnapshot.ts` carries the current threads + `last_run` timestamp; refresh workflow documented in-file (call `mcp__vercel__list_toolbar_threads` in a Claude session, update the file, commit). New page `src/app/admin/feedback/page.tsx`: headline card (unresolved count — green/amber colour shift), thread list with page-path + author + message + comment count + branch + Vercel deep-link, plus a "How to use" section explaining the toolbar workflow (open preview URL → click any element → leave comment). Current state at 2026-05-13: 0 unresolved threads (no comments left yet). Especially valuable for F1 friend-test reopening — testers leave inline feedback in-context rather than via Slack/email. Linked from /admin/health header. Type-check clean. **No new founder action** — the toolbar is already enabled by default in Vercel preview deploys. **Phase B is COMPLETE** after PR J8 (next, process-only): J4, J3, J5 all shipped. **Next**: PR J8 — security/review skill adoption (process change, zero LOC). -->
<!-- 9.27: PR J3 — Stripe revenue admin surface at `/admin/revenue`. Brings the data the founder currently checks externally on the Stripe dashboard inside the admin pane next to /admin/health + /admin/ai-cost + /admin/retention. New files: `src/app/admin/revenue/page.tsx` (admin-gated; reads via existing `getStripe()` singleton + `getFoundingCount()` helper from `src/lib/stripe.ts`) + `src/app/admin/revenue/RevenueDashboard.tsx`. Headline cards: MRR in £ (annual subs normalised to monthly /12), Active subs count, Trialing count, Refunds last 30d. Founding-tier progress bar (count/cap with remaining + colour shift when <50 left). Recent payments list (last 10 charges in 30d window with status badge + amount + customer/desc + ISO date). Promo-code creation deliberately deferred to a follow-on PR — needs a POST route with confirmation flow to avoid accidental coupon creation. Note in the page footer points to the Stripe MCP (`create_coupon`, `create_payment_link`) as the Claude-side path for one-off operations. Linked from /admin/health header (new "Revenue" link). Graceful no-op when `STRIPE_SECRET_KEY` is unset (shows setup prompt). Type-check clean. **No new founder action** — Stripe is already wired (existing /api/stripe/* routes); the dashboard reads from the same credentials. **Next**: PR J5 — Vercel toolbar feedback admin surface. -->
<!-- 9.26: PR J4 — Sentry deepening (release tags + recent-events admin card + Seer workflow doc). (1) **Release tracking** — all three Sentry init files (`sentry.client.config.ts`, `sentry.server.config.ts`, `sentry.edge.config.ts`) now pass `release: process.env.VERCEL_GIT_COMMIT_SHA` so every captured event lands tagged with the deploy that introduced it. Sentry's Releases tab can now diff regressions to a specific PR; before J4, events went into the generic "no release" bucket. (2) **Recent events card on /admin/health** — when `SENTRY_AUTH_TOKEN` is set, the dashboard fetches the 5 most-recent unresolved issues from the Sentry Web API (`/api/0/organizations/nextsplot/issues/?statsPeriod=24h&query=is:unresolved&limit=5`) and renders them with level badge + count + click-through to the Sentry issue page. When the token is missing, the card shows a setup prompt explaining how to mint one (Sentry Settings → Account → API → Auth Tokens, scoped to `project:read event:read`). New env vars in `src/lib/config.ts`: `SENTRY_AUTH_TOKEN`, `SENTRY_ORG_SLUG` (default `nextsplot`), `SENTRY_PROJECT_SLUG`, `SENTRY_REGION_URL` (default `https://de.sentry.io` per the org's region). (3) **Seer workflow doc** — the events card's footer points to `mcp__sentry__analyze_issue_with_seer` as the Claude-side root-cause tool for any flagged issue. Type-check clean. **Founder action (J4, ~3 min)**: in Sentry dashboard → Settings → Account → API → Auth Tokens, create a new token scoped to `project:read event:read` (org-level), copy it, set `SENTRY_AUTH_TOKEN` on Vercel (Production + Preview). Optionally set `SENTRY_PROJECT_SLUG` to scope events to a single project. **Next**: PR J3 — Stripe revenue dashboard. -->
<!-- 9.25: PR J14 — Inngest cron infrastructure ready, founder activation pending. Vercel Hobby caps daily crons at 2/2 (smart-notify + race-tick are both already used). Any 3rd cron (PR J1 advisor sweep, PR J12 weather alert, PR J15 lifecycle digest) requires Vercel Pro OR migration to a separate cron driver. J14 ships the Inngest path so future scheduled jobs slot in without budget pressure. New files: `src/inngest/client.ts` (Inngest singleton + `isInngestConfigured()` + `isInngestPrimary()` guards); `src/inngest/functions.ts` (2 function definitions mirroring the existing Vercel crons — `smart-notify-daily` at 0 14 * * * + `race-tick-daily` at 5 22 * * *; each function fetches the existing `/api/cron/*` route with `x-source: inngest` header rather than duplicating route logic); `src/app/api/inngest/route.ts` (serve handler — signingKey auto-detected from INNGEST_SIGNING_KEY env, returns 401 to unsigned callers when env unset). Existing cron routes (`smart-notify` + `race-tick`) gain a double-fire guard: when `INNGEST_PRIMARY=true` AND the request lacks `x-source: inngest`, the route returns 200 with `{skipped: 'inngest-primary'}` — lets Vercel cron continue pinging during cutover without doing duplicate work. Dep: `inngest@4.4.0`. Type-check clean. **Founder action (J14, ~10 min)**: (a) sign up at app.inngest.com (free 50k events/mo); (b) create a new "App" pointing at https://nextsplit.app/api/inngest; (c) copy the signing key + event key from Inngest dashboard → Settings; (d) set `INNGEST_EVENT_KEY` + `INNGEST_SIGNING_KEY` + `INNGEST_PRIMARY=true` on Vercel (Production + Preview); (e) redeploy; (f) in Inngest dashboard, click "Sync app" — should detect the 2 functions; (g) optionally delete the `crons` block from `vercel.json` once Inngest fires confirmed. Until step (d) is done, the code runs unchanged Vercel-cron path — zero regression. **Phase A is COMPLETE** after this PR: J1, J2, J7, J10, J17, J14 all shipped. **Next phase: B (observability + ops surfaces)** starting with PR J4 — Sentry deepening (releases + Seer + events card). -->
