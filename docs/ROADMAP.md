...
7. **P2.7 hard-deload design.** *(Resolved 2026-05-10, founder vote OQ#7=B)* Soft-deload (advisory banner) shipped. Hard-deload "show both" UX shipped via PR T: when ACWR > 1.3 AND chronic ≥ 12 km/wk AND prescribed = high-volume (tempo/interval/long), `<DeloadAlternativeCard />` renders directly under the original session card on /train. Athlete picks. Tapping the deload pre-fills LogModal with a 60%-volume easy alternative (capped 8km, floor 3km). Logging it writes to the same (week_n, day_i, session_i=0) slot so streak/done counts treat it as completed. Analytics: `hard_deload_shown` + `hard_deload_accepted` for conversion tracking. Founder-overridden options A (hide), C (auto-replan), D (advisory-only) noted in OQ#7 vote.
...
- BL-C4 — Coach-Pro Monday digest (cache layer + idempotency key + fan-out, not synchronous) — T4, T8 / F / ✅ SHIPPED — `phase-blc4-coach-digest-runs-v1.sql` adds `coach_digest_runs` table with `UNIQUE (coach_id, period)` idempotency lock + RLS SELECT for coach. `src/lib/monday-digest.ts:runMondayDigest()` aggregates per-athlete signals (sessions done / planned / km / missed flag) over the past 7 days, classifies each as on_track/behind/silent/no_plan, persists the cached payload in `digest_payload jsonb`, fans out via `coachPush` (push + in-app notifications row, type=`coach_monday_digest`, feature=`blc4-monday-digest`). Hooked into `/api/cron/smart-notify` on `getUTCDay() === 1` so it shares the existing 14:00 UTC slot — no second cron needed (Vercel Hobby cap).
- BL-C5 — Delayed-start 14-day trial (begins day 8, overlaps churn window) — T5 / all / ✅ SHIPPED — `phase-blc5-day8-trial-v1.sql` extends `profiles_trial_source_check` to allow `'day8_auto'` + adds service-role-only `grant_day8_auto_trials()` RPC. RPC selects users where signup is 7-30 days old AND no prior trial AND not Pro, flips `trial_started_at = now()` + `trial_source = 'day8_auto'` + `subscription_status = 'trialing'`. `src/lib/trial-lifecycle.ts:runDay8AutoTrialSweep()` runs daily inside smart-notify, dispatches source-aware welcome push for each row. Bounded backfill (30d signup window) prevents the first deploy from retroactively granting trials to dormant accounts. UI surfaces (TrialBanner, TrialWelcomeModal, TrialLapsedBanner, /admin/trials) extended to recognise the new source with copy "Week 1 done — Pro on us".
- BL-C6 — Trial unlock on squad-join OR first coach msg (avoids hard Stripe dependency) — T5 / all / ✅ SHIPPED — `phase-blc6-trial-unlock-v1.sql` adds `profiles.trial_started_at | trial_source ('squad_join'|'first_coach_message') | trial_ended_at` + two SECDEF RPCs (`grant_trial_if_eligible(p_source)`, `expire_trial_if_due()`) idempotent via `trial_started_at IS NULL` guard. `/api/squad/members POST` calls the eligibility RPC after a successful join (auth.uid() bound). `/api/coach/message POST` writes via SERVICE-ROLE admin client when isCoach (athlete is the recipient, so RLS rejects auth-bound writes). `useSubscription` derives `isTrialing` + `trialDaysLeft`, makes Pro-effective when trialing, and fires lazy expiry RPC when the 14-day window has passed. `<TrialBanner />` on Home shows a 3-tier countdown (cyan → amber → ember as days dwindle).
- BL-C6 lifecycle — `phase-blc6-lifecycle-flags-v1.sql` adds `trial_warned_at` + service-role-only `warn_overdue_trials()` / `expire_overdue_trials()` sweep RPCs. `src/lib/trial-lifecycle.ts:runTrialLifecycleSweep()` runs daily inside smart-notify, dispatching: (a) day-13 "1 day left" push (idempotent via `trial_warned_at`), (b) day-14+ "trial ended" push (idempotent via `trial_ended_at`). Welcome push fires inline from the squad-join + coach-message routes when grant flips `trial_started_at` (`sendTrialWelcomePush` with source-aware copy). All three pushes route through `coachPush` so they share `feature='blc6-trial-lifecycle'` Sentry tagging.
